...
export function CartClient({ relatedPlants }: CartClientProps) {
    ...
    if (cart.items.length === 0) {
        return (
            <div className="min-h-[80vh] flex flex-col items-center gap-6 p-4 mt-30 text-center">
                <div className="w-24 h-24 bg-[#FAF9F6] rounded-full flex items-center justify-center">
                    <span className="text-4xl text-[#2D5A42]">🛒</span>
                </div>
                <h1 className="font-serif text-3xl font-bold text-[var(--color-text-primary)]">Your cart is empty</h1>
            ...
            <div className="px-4 md:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                        ...
                        <div className="space-y-2">
                            {cart.items.map((item) => {
                                const price = item.selectedVariant
                                    ...
                                    : (item.plant.discountPrice || item.plant.price);
                                return (
                                    <div
                                        key={`${item.plant.id}-${item.selectedVariant?.id || 'default'}`}
                                        className="flex gap-4 md:gap-8 p-2 md:p-4 bg-[var(--color-surface-hover)] rounded-[10px] border border-primary/50 shadow-sm group relative cursor-pointer"
                                        onClick={() => router.push(`/plants/${item.plant.id}`)}
                                    >
                                        {/* Image */}
                                        ...
                                        {/* Content */}
                                        <div className="flex-1 flex flex-col justify-between md:py-2">
                                            <div className="flex justify-between items-start gap-2">
                                                <div className="space-y-1">
                                                    ...
                                                    <div className='flex gap-2'>
                                                        {item.plant.category && (
                                                            <p className="text-xs md:text-sm text-[var(--color-text-secondary)] italic font-medium opacity-80">{item.plant.category.toLowerCase()}</p>
                                                        )}
                                                        {item.selectedVariant && (
                                                            <p className="text-xs md:text-sm text-[var(--color-text-muted)] font-medium">- Size: {item.selectedVariant.size}</p>
                                                        )}
                                                    </div>
                                                </div>
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        removeFromCart(item.plant.id, item.selectedVariant?.id);
                                                    }}
                                            ...
                                            <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5 md:border-none">
                                                {/* Quantity Selector */}
                                                <div className="flex items-center gap-4 bg-[var(--color-surface)] rounded-full px-2 py-1 shadow-sm border border-primary">
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            updateQuantity(item.plant.id, Math.max(1, item.quantity - 1), item.selectedVariant?.id);
                                                        }}
                                                    ...
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            updateQuantity(item.plant.id, item.quantity + 1, item.selectedVariant?.id);
                                                        }}
                    ...
                    <div className="lg:col-span-4 lg:sticky lg:top-32">
                        <CartSummary totalPrice={cart.totalPrice} totalQuantity={cart.items.length} totalPlants={cart.totalItems} checkoutUrl={checkoutUrl} />
                    </div>
                </div>
                ...
                {relatedPlants.length > 0 && (
                    <div className="mt-6 border-t border-white/10">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 md:gap-8 px-2">
